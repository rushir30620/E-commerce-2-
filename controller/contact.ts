...
export class ContactUserController {
    ...
    public getAllContactUsers = async (req: Request, res: Response, next: NextFunction): Promise<Response> => {
    ...
    public getContactUserById = async (req: Request, res: Response, next: NextFunction): Promise<Response> => {
        return this.contactUserService.getContactUserById(+req.params.id)
            .then((contactUser) => {
                if (contactUser) {
                    return res.status(200).json({ success: true, contactUser, error: {} });
                }
